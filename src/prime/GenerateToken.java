...
public class GenerateToken {
    ...
    /* Issuer will be based on business - FACL(Falabella-Chile), SOCL(Sodimac-Chile), TOCL (Tottus-Chile) */
    ...
    public static String createJWT(ProfileToken profileToken) throws UnsupportedEncodingException {
        ...
        JwtBuilder builder = Jwts.builder().setId(JWT_TOKEN).setIssuedAt(now).setSubject(SUBJECT)
                .setIssuer(FALABELLA_ISSUER).signWith(signatureAlgorithm, signingKey).claim
                        (JTI_FIELD, JTI_VALUE)
                .claim(E_MAIL, profileToken.getEmail()).claim(DOCUMENT_NUMBER, profileToken.getDocumentNumber());
        builder.setExpiration(DateUtils.addHours(now,TOKEN_EXPIRY));
        return builder.compact();
    }

    public static void main(String[] args) throws UnsupportedEncodingException {
        //ProfileToken profile = new ProfileToken("RUT","email");
    	ProfileToken profile = new ProfileToken("19","roopikacfalla@gmail.com");
